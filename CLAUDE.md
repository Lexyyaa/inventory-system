...
| F5 마무리 | 00 / 01 / 02 §1 / `task_list` 보류와 리뷰 백로그 |
...
- 스킬 밖에서 사용자가 말하기 전에 커밋, push, PR, 머지
  - `/run-feature`와 `/wrap-up` 안의 작업 커밋은 스킬 실행이 승인이다. push와 PR은 스킬의 멈춤 지점에서 묻는다
...
- 비즈니스 규칙과 불변식은 도메인 객체와 도메인 서비스에. ApplicationService는 조합과 트랜잭션만
