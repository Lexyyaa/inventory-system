...
3. F2 코드 · 규칙 조정
   ...
   - `5e66c0f` 스킬 · doc-writer 지시에서 작업 시간 기록 삭제
4. reviewer (verifier 생략)
   - `0540620` task_list 시간 계획 표 · 시간 목표 삭제
5. 서버 실측
...
- 문서
  ...
  - `src/main/CLAUDE.md` (수정) — 엔티티 규칙 · 주석 자리
  - `src/test/CLAUDE.md` (수정) — 테스트 데이터는 DB 직접 삽입
  - `.claude/skills/` (design · run-feature · wrap-up) · `.claude/agents/` (doc-writer · analyst) (수정) — 작업 시간 기록 · 목표 삭제
  - `docs/ai-log/F2-inbound.md` (수정) — 업체 확인을 `TenantService`로 나눈 내용 추가
  - `docs/task_list.md` (수정) — 시간 계획 표 · 시간 목표 삭제
...
- 05 F3 · F4 given (`c7ac256`)
  ...
  - 이유: 입고가 깨져도 조회 · 출고 테스트가 같이 깨지지 않게
