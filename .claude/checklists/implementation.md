...
- [ ] 계산과 검증은 DTO가 아니라 도메인 메서드에 있다 — 규칙이 흩어지고 도메인 테스트로 못 잡는다
- [ ] "무시한다"고 정한 입력 필드에 Bean Validation이 남아 있지 않다 — 무시될 값 때문에 400
- [ ] `Info`와 `Command`에 domain Enum, VO, 엔티티를 담지 않는다 — presentation이 domain을 참조해 ArchUnit 실패 (`name()`/원시값으로 푼다)
...
- [ ] PostgreSQL에 저장할 수 없는 NUL과 짝 없는 서로게이트는 Request에서 400으로 막는다 — NUL은 500, 서로게이트는 다른 문자로 저장돼 다음 입고가 409
- [ ] 컨트롤러 매핑에 `consumes`를 두지 않는다 — Content-Type 오류가 업체 확인보다 먼저 400 `INVALID_REQUEST`가 되어 04 §2 순서가 뒤집힌다 (TC-2-08 요청 4)
- [ ] 컨트롤러 매핑의 `produces`는 JSON으로 둔다 — 없으면 Accept 협상이 커밋 뒤에 일어나 재고는 바뀌었는데 406이 나간다
- [ ] 쓰지 않는 프레임워크 예외 매핑을 지우거나 설정을 바꿀 때는 실서버 curl로 확인한다 — MockMvc는 multipart 본문을 파싱하지 않아 500을 400으로 통과시킨다
- [ ] 출고의 `INVALID_QUANTITY`는 출고 문구("출고 수량이 허용 범위를 벗어났습니다.")를 `detail`로 넘긴다 — `ErrorCode` 기본 문구가 입고 문구라 빠뜨리면 출고 응답에 "입고 수량이 …"가 나간다
- [ ] 컨트롤러 매핑에 `/api/v1`을 쓰지 않는다 — 접두사는 `WebConfig.API_PREFIX`가 붙여서 `/api/v1/api/v1/...`이 된다
...
- [ ] 시각 컬럼은 DB가 채우고, 엔티티는 매핑 전용 `BaseTimeEntity`(Auditing 없음, `Instant`)로 읽기만 한다 — Auditing은 native 쿼리 저장에 적용되지 않아 값이 비거나 서버 시각으로 과거로 돌아간다

## 락, 트랜잭션, 동시성

- [ ] 트랜잭션은 ApplicationService 메서드 하나에 하나다. 같은 클래스 안의 `@Transactional` 호출에 기대지 않는다 — 프록시를 안 타서 트랜잭션이 안 열린다
...
- [ ] 조회 서비스는 `@Transactional(readOnly = true)`다 — 불필요한 flush와 쓰기 락
...
- [ ] 부분 성공 API를 상태코드만이 아니라 본문과 DB 건수로 검증한다 — 200인데 저장 0건인 거짓 통과
...
- [ ] 주석에 문서 번호(`03 §9`, `TC-2-05`)와 `{@code}`, `{@link}`, `<p>`가 없다. 필요하면 "왜"만 한 줄 요약 + 불릿, 줄 끝 `<br>` — 문서와 주석 두 곳에 같은 말이 생기고 문서가 바뀌면 주석이 거짓이 된다
