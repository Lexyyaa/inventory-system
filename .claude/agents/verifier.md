...
- 분석 §2 위험 문장 심문과 기능 간 교차의 깨지는 시나리오마다 대응하는 TC가 있거나 범위 밖으로 적혔는가
...
- 모든 `FR`과 `NFR`이 `task_list`의 작업 줄에 한 번 이상 나오는가
...
- Request 필드와 검증 어노테이션 ↔ `04` 각 API의 필드와 검증
- Response 필드 ↔ `04` 각 API의 성공 응답 (내부 id 없음, `updatedAt` +09:00)
- `ErrorCode`의 HTTP 상태와 메시지 ↔ `04` §7과 각 API의 오류 응답
- 오류 판정 순서 ↔ `04` §2
- `schema.sql` 제약과 엔티티 매핑 ↔ `03` §7
- 원자 SQL(상품 생성, 재고 증가, 재고 감소, 조회) ↔ `03` §9~§11, §13
- 트랜잭션 경계와 격리 수준 ↔ `03` §14, §15
...
## 감사 커버리지
- 대조한 ADR: n / 전체
- 방향 1: 대조한 TC와 규칙 n / 완료 표시된 기능의 TC와 규칙 전체, 미확인 [ID]
